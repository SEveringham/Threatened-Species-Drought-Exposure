...
p50threatenedgraph <- droughtandxylemdatarobustp50 %>%
  ...
  ggplot(aes(x = p50, y = redlistCategory.x, fill = redlistCategory.x)) +
...
```{r}
saeftymarginthreatenedgraph <- droughtandxylemdatarobustsafetymargin %>%
  mutate(redlistCategory.x = if_else(.$redlistCategory.x %in% c('Critically Endangered','Endangered', 'Vulnerable'), "threatened", "ynot threatened")) %>%
  ggplot(aes(x = safetymargin, y = redlistCategory.x, fill = redlistCategory.x)) +
  geom_density_ridges(scale = 0.7, alpha = 0.8, quantile_lines = T, quantile_fun = mean,
    jittered_points = TRUE,
    vline_size = 1, vline_color = "black",
    point_size = 1.2, point_alpha = 0.2, point_color = "black", 
