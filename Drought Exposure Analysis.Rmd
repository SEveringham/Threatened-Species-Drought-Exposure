...
threatenedspeciesprop245 <- droughtdatarobust %>%
  ...
  ggplot(aes(x = positiveornegative, y = log(proportion+0.00001), fill = category.x)) +
  ...
  stat_summary(
    ...
    geom = "pointrange", color = "black") +
    scale_fill_manual(name = "category.x", values = c("#53619E", "#7D5D3A", "yellow3")) +
    theme_classic() +
...
Density plots of duration change (not just a binary negative or positive)
