...
droughtdatafull <- left_join(droughtdatafull, growthform, by = 'scientificName') %>%
  filter(!Aquatic == "yes") #subset just without aquatic plants - this gives us 65746 non-aquatic species on IUCN

droughtdata <- droughtdatafull %>%
  filter(EOODataAvailable == "AvailableEOO") #then subset species with EOO data - gives us 10131 species 

pointdroughtdata <- droughtdatafull %>%
  filter(EOODataAvailable == "AvailablePoint") #then subset species with AOO data - gives us 36262 species
```
...
threatdata <- droughtdatarobust %>%
  ...
  mutate(percentage = (total/7647)*100)
