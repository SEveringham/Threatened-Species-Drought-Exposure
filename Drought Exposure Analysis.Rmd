...
sum(droughtdata$duration_245_proportion_non_robust.x == 0)
#1808 species we cannot use!
...
sum(droughtdata$duration_585_proportion_non_robust.x == 0)
#3324 species we cannot use for scenario 3324!

#quick count of how many species we can't use altoghter
sum(droughtdata$duration_245_proportion_non_robust.x == 0 &
      droughtdata$duration_585_proportion_non_robust.x == 0)
#1796 species have no range in robust drought projection areas for either scenario

#remove these species where there is no range at all in robust areas
droughtdatarobust <- droughtdata %>%
  filter(!(droughtdata$duration_245_proportion_non_robust.x == 0 &
      droughtdata$duration_585_proportion_non_robust.x == 0))
```

Now we are left with 7950 species with extents of occurrence in at least a small proportion of ther range in a robust drought projected area for either scenario
