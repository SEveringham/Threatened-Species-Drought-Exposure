...
droughtdatasaved <- read.csv("Data/Data Used in Analyses/fulldroughtwithxylemdata.csv") %>%
  mutate(frequency_585_proportion_non_robust = as.numeric(frequency_585_proportion_non_robust)) %>% 
  mutate(frequency_585_proportion_negative_value = as.numeric(frequency_585_proportion_negative_value)) %>%
  mutate(frequency_245_mean = as.numeric(frequency_245_mean))# some columns with a few missing values being read as character - fixed this
...
robustnessofdroughtprojection585 <- ggplot(data=droughtdata, aes(x =frequency_585_proportion_non_robust, fill = "#53619E")) +
  geom_histogram(stat = "count", colour = "grey40", bins = 100) +
  ggplot2::annotate("text",
...
#save this data- may need just a matrix of species with exposure change values for both scenarios (and only save threatened species)
droughtexposuredata <- droughtdatarobust %>%
  select(scientificName, redlistCategory.x, eoo_km2, duration_245_mean, duration_245_proportion_positive_value, duration_245_proportion_negative_value, duration_585_mean, duration_585_proportion_positive_value, duration_585_proportion_negative_value, frequency_245_mean, frequency_245_proportion_positive_value, frequency_245_proportion_negative_value, frequency_585_mean, frequency_585_proportion_positive_value, frequency_585_proportion_negative_value, p50) %>%
  filter(redlistCategory.x %in% c("Critically Endangered", "Endangered", "Vulnerable"))

#save as csv
write.csv(droughtexposuredata, "Data/Data Used in Analyses/cleanedexposurematrix.csv")
```
...
- 96.4% of species (2125 species out of 2205) have some part of their range that falls into at least one area of an increase in drought frequency
- 67.6% of species (1490 out of 2205) have their entire range falling into areas that increase in drought frequency
- 4.3% of species (95 out of 2205) have some part of their range that falls into at least one area of a decrease in drought frequency
...
- 95.6% of species (2273 species out of 2380) have some part of their range that falls into at least one area of an increase in drought duration
- 78.9% of species (1877 out of 2380) have their entire range falling into areas that increase in drought duration
- 5.2% of species (126 out of 2380) have some part of their range that falls into at least one area of a decrease in drought duration
- Only 25 species (1.0%) have their entire range falling into regions with decreases in drought frequency

br>
We also need to determine the AVERAGE proportion of any species range that falls into drought increasing/decreasing regions (where species that have entire ranges falling into certain scenarios are excluded)
```{r}
proportiondroughtfrequencyincreasingordecreasingmeans <- BothScenariosData %>%
  group_by(SSP, positiveornegative) %>%
  summarise(mean = mean(proportion, na.rm = T))
```

This data shows us that on average, species will:
- Under SSP2-4.5 have around 0.25 of their range in drought duration decreasing regions and 0.58 of their range in drought increasing regions
- Under SSP5-8.5 have around 0.31 of their range in drought duration decreasing regions and 0.46 of their range in drought increasing regions

<br>


Now same violin plots but with JUST threatened species (vulnerable, endangered, critically endangered) and comparing these groups
```{r,max.height='200px', fig.show="hold", out.width="50%", message = F, warning = F}
#scenario SPP 2-4.5
threatenedspeciesprop245 <- droughtdatarobust %>%
...
## Pulling the total number of species that have at least one pixel of drought INCREASE in any scenario for either duration or frequency projections (i.e. how many species out of the total species with any form of robust drought projection will be exposed to increases in drought AT ALL)

```{r}
totalspeciesexposedtoincreasesindrought <- droughtdatarobust %>%
  filter(!redlistCategory.x == 'Data Deficient' & !redlistCategory.x == 'Extinct in the Wild' & !redlistCategory.x == 'Extinct' & !redlistCategory.x == 'Least Concern' & !redlistCategory.x == 'Near Threatened' & !redlistCategory.x == 'Lower Risk/near threatened' & !is.na(redlistCategory.x)) %>%
  filter(duration_245_proportion_positive_value > 0 | duration_585_proportion_positive_value > 0 | frequency_245_proportion_positive_value > 0 | frequency_585_proportion_positive_value > 0)

(2765/2886)*100
```
2765 threatened species out of 2886 threatened species will be exposed to some kind of drought increase (duration/frequency) under either moderate or high emissions scenarios which is 95.8% of threatened species!

## Determining if the species that have drought listed on their Red List assessment are actually more exposed to future drought

### First calculate the proportions of species that are 'highly exposed to drought' that don't have drought on their Red List assessment

'highly exposed to drought' are those species' whose entire ranges are robust and fall into areas of drought increase

#### For drought duration
```{r}
durationdroughtlisted245 <- droughtdatarobust %>%
  filter(duration_245_proportion_non_robust == 0) %>%
  mutate(highlyexposed = ifelse(duration_245_proportion_positive_value == 1, "yes", "no")) %>%
  filter(highlyexposed == "yes") %>%
  group_by(DroughtThreatened) %>%
  summarise(numberofspeciesthreated = length(DroughtThreatened))

durationdroughtlisted585 <- droughtdatarobust %>%
  filter(duration_585_proportion_non_robust == 0) %>%
  mutate(highlyexposed = ifelse(duration_585_proportion_positive_value == 1, "yes", "no")) %>%
  filter(highlyexposed == "yes") %>%
  group_by(DroughtThreatened) %>%
  summarise(numberofspeciesthreated = length(DroughtThreatened))  
```
...
```{r}
frequencydroughtlisted245 <- droughtdatarobust %>%
  filter(frequency_245_proportion_non_robust == 0) %>%
  mutate(highlyexposed = ifelse(frequency_245_proportion_positive_value == 1, "yes", "no")) %>%
  filter(highlyexposed == "yes") %>%
  group_by(DroughtThreatened) %>%
  summarise(numberofspeciesthreated = length(DroughtThreatened))

frequencydroughtlisted585 <- droughtdatarobust %>%
  filter(frequency_585_proportion_non_robust == 0) %>%
  mutate(highlyexposed = ifelse(duration_585_proportion_positive_value == 1, "yes", "no")) %>%
  filter(highlyexposed == "yes") %>%
  group_by(DroughtThreatened) %>%
  summarise(numberofspeciesthreated = length(DroughtThreatened))  
...
#### Run linear model for Drought duration

```{r}
#for SSP2-4.5
#subset data
duration245threatmodeldata <- droughtdatarobust %>%
  filter(redlistCategory.x == 'Critically Endangered' & !redlistCategory.x == 'Endangered' & !redlistCategory.x == 'Vulnerable' & !is.na(redlistCategory.x))
#run linear model
duration245threatmodel <-  lm(duration_245_mean ~ as.factor(DroughtThreatened), data = duration245threatmodeldata)
#check assumptions
plot(duration245threatmodel)
#get model output
summary(duration245threatmodel)

#for SSP5-8.5
duration585threatmodeldata <- droughtdatarobust %>%
  filter(redlistCategory.x == 'Critically Endangered' & !redlistCategory.x == 'Endangered' & !redlistCategory.x == 'Vulnerable' & !is.na(redlistCategory.x))
#run linear model
duration585threatmodel <-  lm(duration_585_mean ~ as.factor(DroughtThreatened), data = duration585threatmodeldata)
#check assumptions
plot(duration585threatmodel)
#get model output
summary(duration585threatmodel)
```


#### Plot Drought duration
```{r}
durationanddroughtthreat245 <- droughtdatarobust %>%
  filter(redlistCategory.x == 'Critically Endangered' & !redlistCategory.x == 'Endangered' & !redlistCategory.x == 'Vulnerable' & !is.na(redlistCategory.x)) %>%
  ggplot(aes(x = duration_245_mean, y = DroughtThreatened, fill = DroughtThreatened)) +
  geom_density_ridges(scale = 0.7, alpha = 0.8, quantile_lines = T, quantile_fun = mean,
    jittered_points = TRUE,
    vline_size = 1, vline_color = "black",
    point_size = 1.2, point_alpha = 0.2, point_color = "black", 
  ...
  scale_x_continuous(expand = c(0, 0)) +   # for both axes to remove unneeded padding
  coord_cartesian(clip = "off") + # to avoid clipping of the very top of the top ridgeline
  scale_fill_manual(values = c("#D52001", "#FC7F3F")) +
  guides(fill = "none") +
  xlab("Mean Drought Duration Change for SSP 2-4.5 Projection (Events per 10y)") +
  ylab("") +
  theme_ridges() +
  geom_vline(xintercept = 0, size =1, color = "black", linetype = "dashed") +
  xlim(-1,2.5)

durationanddroughtthreat245

durationanddroughtthreat585 <- droughtdatarobust %>%
  filter(redlistCategory.x == 'Critically Endangered' & !redlistCategory.x == 'Endangered' & !redlistCategory.x == 'Vulnerable' & !is.na(redlistCategory.x)) %>%
  ggplot(aes(x = duration_585_mean, y = DroughtThreatened, fill = DroughtThreatened)) +
  geom_density_ridges(scale = 0.7, alpha = 0.8, quantile_lines = T, quantile_fun = mean,
    jittered_points = TRUE,
    vline_size = 1, vline_color = "black",
    point_size = 1.2, point_alpha = 0.2, point_color = "black", 
...
durationanddroughtthreat585
```


#### Run linear model for Drought frequency

```{r}
#for SSP2-4.5
#subset data
frequency245threatmodeldata <- droughtdatarobust %>%
  filter(redlistCategory.x == 'Critically Endangered' & !redlistCategory.x == 'Endangered' & !redlistCategory.x == 'Vulnerable' & !is.na(redlistCategory.x))
#run linear model
frequency245threatmodel <-  lm(frequency_245_mean ~ as.factor(DroughtThreatened), data = frequency245threatmodeldata)
#check assumptions
plot(frequency245threatmodel)
#get model output
summary(frequency245threatmodel)

#for SSP5-8.5
frequency585threatmodeldata <- droughtdatarobust %>%
  filter(redlistCategory.x == 'Critically Endangered' & !redlistCategory.x == 'Endangered' & !redlistCategory.x == 'Vulnerable' & !is.na(redlistCategory.x))
#run linear model
frequency585threatmodel <-  lm(frequency_585_mean ~ as.factor(DroughtThreatened), data = frequency585threatmodeldata)
#check assumptions
plot(frequency585threatmodel)
#get model output
summary(frequency585threatmodel)
```
...
```{r}
frequencyanddroughtthreat245 <- droughtdatarobust %>%
  filter(redlistCategory.x == 'Critically Endangered' & !redlistCategory.x == 'Endangered' & !redlistCategory.x == 'Vulnerable' & !is.na(redlistCategory.x)) %>%
  ggplot(aes(x = frequency_245_mean, y = DroughtThreatened, fill = DroughtThreatened)) +
  geom_density_ridges(scale = 0.7, alpha = 0.8, quantile_lines = T, quantile_fun = mean,
    jittered_points = TRUE,
    vline_size = 1, vline_color = "black",
    point_size = 1.2, point_alpha = 0.2, point_color = "black", 
...
frequencyanddroughtthreat245

frequencyanddroughtthreat585 <- droughtdatarobust %>%
  filter(redlistCategory.x == 'Critically Endangered' & !redlistCategory.x == 'Endangered' & !redlistCategory.x == 'Vulnerable' & !is.na(redlistCategory.x)) %>%
  ggplot(aes(x = frequency_585_mean, y = DroughtThreatened, fill = DroughtThreatened)) +
  geom_density_ridges(scale = 0.7, alpha = 0.8, quantile_lines = T, quantile_fun = mean,
    jittered_points = TRUE,
    vline_size = 1, vline_color = "black",
    point_size = 1.2, point_alpha = 0.2, point_color = "black", 
...
```{r, fig.show="hold", out.width="50%", message = F}
robustnessofdroughtprojection245dur <- ggplot(data=pointdroughtdata, aes(x =duration_245_proportion_non_robust, fill = "#53619E")) +
  geom_histogram(colour = "grey40", bins = 100) +
  ggplot2::annotate("text",
    label="a) SSP2-4.5 - duration", 
    x=0.34,
...
robustnessofdroughtprojection585dur <- ggplot(data=pointdroughtdata, aes(x =duration_585_proportion_non_robust,fill = "#53619E")) +
  geom_histogram(colour = "grey40", bins = 100) +
  ggplot2::annotate("text",
    label="b) SSP5-8.5 - duration", 
    x=0.34,
...
ggsave(plot= proportiondroughtdurationincreasingordecreasingviolin, filename = "Outputs/DurationViolinpoints.jpeg",  device = "jpeg", width = 10, height = 7)
...
ggsave(plot= durationdotplot, filename = "Outputs/DurationDotPlotpoints.jpeg",  device = "jpeg", width = 10, height = 9)
```
<br>
Some key stats from above figures:
...
- 98.2% of species (2545 species out of 2592) have some part of their range that falls into at least one area of an increase in drought duration
- 73.5% of species (1905 out of 2592) have their entire range falling into areas that increase in drought duration
- 2% of species (49 out of 2592) have some part of their range that falls into at least one area of a decrease in drought duration
...
- 98.6% of species (5447 species out of 5522) have some part of their range that falls into at least one area of an increase in drought duration
- 30% of species (1656 out of 5522) have their entire range falling into areas that increase in drought duration
- 1% of species (61 out of 5522) have some part of their range that falls into at least one area of a decrease in drought duration
...
```{r, fig.show="hold", out.width="50%", message = F, warning = F}
#scenario SPP 2-4.5
durationdensity245 <- pointdroughtdatarobust %>%
  filter(!redlistCategory.x == 'Data Deficient' & !redlistCategory.x == 'Extinct in the Wild' & !redlistCategory.x == 'Extinct' & !is.na(redlistCategory.x)) %>%
  mutate(redlistCategory.x= if_else(.$redlistCategory.x %in% c('Critically Endangered','Endangered', 'Vulnerable'), redlistCategory.x, "ynot threatened")) %>%
  ggplot(aes(x = duration_245_mean, y = redlistCategory.x, fill = redlistCategory.x)) +
  geom_density_ridges(scale = 0.7, alpha = 0.8, quantile_lines = T, quantile_fun = mean,
...
ggsave(plot= durationdensity245, filename = "Outputs/Durationdensity245point.jpeg",  device = "jpeg", width = 10, height = 7)
ggsave(plot= durationdensity585, filename = "Outputs/Durationdensity585point.jpeg",  device = "jpeg", width = 10, height = 7)
```
...
```{r}
#filter and create levels for the four categories of data (not threatened, vulnerable, endangered, critically endangered)
duration245dataformodel <- pointdroughtdatarobust %>%
  filter(!redlistCategory.x == 'Data Deficient' & !redlistCategory.x == 'Extinct in the Wild' & !redlistCategory.x == 'Extinct' & !is.na(redlistCategory.x)) %>%
  mutate(redlistCategory.x= ifelse(redlistCategory.x == 'Vulnerable', '2', 
                                           ifelse(redlistCategory.x == 'Endangered', '3',
                                                  ifelse(redlistCategory.x == 'Critically Endangered', '4',
...
#SSP 5-8.5
SpeciesScenariosfrequency585 <- pointdroughtdatarobust %>%
  filter(redlistCategory.x == 'Critically Endangered' | redlistCategory.x == 'Endangered' | redlistCategory.x == 'Vulnerable') %>%
  filter(!frequency_585_proportion_non_robust ==1) %>%
  mutate(Scenario = ifelse(frequency_585_proportion_positive_value == 1, " increaseAll",
                           ifelse(frequency_585_proportion_positive_value > 0 & frequency_585_proportion_positive_value < 1 & frequency_585_proportion_negative_value ==0, " increaseSome",
...
ggsave(plot= frequencydotplot, filename = "Outputs/frequencyDotPlotpoint.jpeg",  device = "jpeg", width = 10, height = 9)
```
<br>
...
- 90.1% of species (4749 species out of 5269) have some part of their range that falls into at least one area of an increase in drought frequency
- 33.5% of species (1763 out of 5269) have their entire range falling into areas that increase in drought frequency
- 10.2% of species (539 out of 5269) have some part of their range that falls into at least one area of a decrease in drought frequency
- 83 species (2%) has their entire range falling into regions with decreases in drought frequency
...
- 90.5% of species (5594 species out of 6183) have some part of their range that falls into at least one area of an increase in drought duration
- 75.8% of species (4686 out of 6183) have their entire range falling into areas that increase in drought duration
- 9.8% of species (609 out of 6183) have some part of their range that falls into at least one area of a decrease in drought duration
- 3% (162 out of 6183 species) have their entire range falling into regions with decreases in drought duration
...
ggsave(plot= frequencydensity245, filename = "Outputs/Frequencydensity245point.jpeg",  device = "jpeg", width = 10, height = 7)
ggsave(plot= frequencydensity585, filename = "Outputs/Frequencydensity585point.jpeg",  device = "jpeg", width = 10, height = 7)
```
#### Testing differences between threatened and non-threatened plants
...
```{r}
#filter and create levels for the four categories of data (not threatened, vulnerable, endangered, critically endangered)
frequency245dataformodel <- pointdroughtdatarobust %>%
  filter(!redlistCategory.x == 'Data Deficient' & !redlistCategory.x == 'Extinct in the Wild' & !redlistCategory.x == 'Extinct' & !is.na(redlistCategory.x)) %>%
  mutate(redlistCategory.x= ifelse(redlistCategory.x == 'Vulnerable', '2', 
                                           ifelse(redlistCategory.x == 'Endangered', '3',
                                                  ifelse(redlistCategory.x == 'Critically Endangered', '4',
...
```{r}
meansofdroughtchangepointdata <- pointdroughtdatarobust %>%
  filter(!redlistCategory.x == 'Data Deficient' & !redlistCategory.x == 'Extinct in the Wild' & !redlistCategory.x == 'Extinct' & !is.na(redlistCategory.x)) %>%
  mutate(threatened = if_else(.$redlistCategory.x %in% c('Critically Endangered','Endangered', 'Vulnerable'), "threatened", "not threatened")) %>%
  group_by(threatened) %>%
  summarise(averageduration245 = mean(duration_245_mean, na.rm = T),
...
## Repeating the testing differences between threatened and non-threatened plants in drought exposure with ONLY species in drought increasing regions
...
duration245dataformodel <- droughtdatarobust %>%
  ...
  mutate(redlistCategory.x= ifelse(redlistCategory.x == 'Vulnerable', '2', 
                                           ifelse(redlistCategory.x == 'Endangered', '3',
                                                  ifelse(redlistCategory.x == 'Critically Endangered', '4',
                                                         if_else(.$redlistCategory.x %in% c('Critically Endangered','Endangered', 'Vulnerable'), redlistCategory.x, '1'))))) %>%
  filter(duration_245_mean > 0) #filtering for just species with average INCREASES in drought duration
...
```{r}
#filter and create levels for the four categories of data (not threatened, vulnerable, endangered, critically endangered)
duration585dataformodel <- droughtdatarobust %>%
  filter(!redlistCategory.x == 'Data Deficient' & !redlistCategory.x == 'Extinct in the Wild' & !redlistCategory.x == 'Extinct' & !is.na(redlistCategory.x)) %>%
  mutate(redlistCategory.x= ifelse(redlistCategory.x == 'Vulnerable', '2', 
                                           ifelse(redlistCategory.x == 'Endangered', '3',
                                                  ifelse(redlistCategory.x == 'Critically Endangered', '4',
                                                         if_else(.$redlistCategory.x %in% c('Critically Endangered','Endangered', 'Vulnerable'), redlistCategory.x, '1'))))) %>%
  filter(duration_585_mean > 0) #filtering for just species with average INCREASES in drought duration


#run linear model
duration585model <- lm(duration_585_mean ~ as.factor(redlistCategory.x), data = duration585dataformodel)
#check assumptions
plot(duration585model)
#get model output
summary(duration585model)
...
frequency245dataformodel <- droughtdatarobust %>%
  ...
  mutate(redlistCategory.x= ifelse(redlistCategory.x == 'Vulnerable', '2', 
                                           ifelse(redlistCategory.x == 'Endangered', '3',
                                                  ifelse(redlistCategory.x == 'Critically Endangered', '4',
                                                         if_else(.$redlistCategory.x %in% c('Critically Endangered','Endangered', 'Vulnerable'), redlistCategory.x, '1'))))) %>%
  filter(frequency_245_mean > 0) #filtering for just species with average INCREASES in drought frequency
...
```{r}
#filter and create levels for the four categories of data (not threatened, vulnerable, endangered, critically endangered)
frequency585dataformodel <- droughtdatarobust %>%
  filter(!redlistCategory.x == 'Data Deficient' & !redlistCategory.x == 'Extinct in the Wild' & !redlistCategory.x == 'Extinct' & !is.na(redlistCategory.x)) %>%
  mutate(redlistCategory.x= ifelse(redlistCategory.x == 'Vulnerable', '2', 
                                           ifelse(redlistCategory.x == 'Endangered', '3',
                                                  ifelse(redlistCategory.x == 'Critically Endangered', '4',
                                                         if_else(.$redlistCategory.x %in% c('Critically Endangered','Endangered', 'Vulnerable'), redlistCategory.x, '1'))))) %>%
  filter(frequency_585_mean > 0) #filtering for just species with average INCREASES in drought frequency

#run linear model
frequency585model <- lm(frequency_585_mean ~ as.factor(redlistCategory.x), data = frequency585dataformodel)
#check assumptions
plot(frequency585model)
#get model output
summary(frequency585model)
