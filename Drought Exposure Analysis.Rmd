...
## Read in data
```{r}
droughtdatafull <- read.csv("Data/Data Used in Analyses/fulldroughtwithxylemdata.csv")
growthform <- read.csv("Data/Data Used in Analyses/fullIUCNandgrowthform.csv")

droughtdatafull <- left_join(droughtdatafull, growthform, by = 'scientificName') %>%
  filter(!Aquatic == "yes")
...
pointdroughtdata <- droughtdatafull %>%
  filter(EOODataAvailable == "AvailablePoint")
```
...
SpeciesScenariosCountfrequency585 <- SpeciesScenariosfrequency585 %>%
  ...
  mutate(Metric = "frequency") %>%
  rename(NumberofSpecies = `length(frequency_585_proportion_non_robust)`) %>%
  mutate(SSP = "585") %>%
  mutate(total = 2323)
...
ggsave(plot= frequencydotplot, filename = "Outputs/frequencyDotPlot.jpeg",  device = "jpeg", width = 8, height = 9)
```
<br>
