...
xylemtraits <- read.csv("Data/Raw Data/Xylem Functional Traits.csv", na.strings=c("","NA")) %>%
  ...
  summarise(p50 = mean(as.numeric(P50), na.rm=T),
            psiminmidday = mean(as.numeric(psi.min.predawn..MPa.), na.rm = T),
            psiminpredawn = mean(as.numeric(psi..min.midday..MPa.), na.rm = T))
...
- only 136 of these have EOO data as well and only 12 are threatened
