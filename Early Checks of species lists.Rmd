...
xylemtraits <- read.csv("Data/Raw Data/Xylem Functional Traits.csv", na.strings=c("","NA")) %>%
  ...
  summarise(p50 = mean(as.numeric(P50..MPa.), na.rm=T),
            psiminmidday = mean(as.numeric(X.p.min.midday..MPa.), na.rm = T),
            psiminpredawn = mean(as.numeric(X.p.min.predawn..MPa.), na.rm = T))
...
- only 373 species (of ~66000) on the IUCN have any p50 data
- only 73 of these have EOO data as well and only 9 are threatened
